  ...
  return rows
    .map((row, index) => {
      ...
      return {
        ...
        launchDirection: safeParse(getRowValue(row, ['Launch Direction', 'Launch H', 'Launch direction'])),
        spinRate,
        backSpin,
        sideSpin,
        spinAxis,
        ...
        totalOffline: 0,
        sessionId,
        clubPath: safeParse(getRowValue(row, ['Club Path', 'Path'])),
        clubFace: safeParse(getRowValue(row, ['Face Angle', 'Club Face', 'Face'])),
      };
    })
