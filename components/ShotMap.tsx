...
const ShotMap: React.FC<ShotMapProps> = ({ shots }) => {
  ...
  const renderTooltip = (shot: Shot) => {
    ...
    return (
      <div 
        className="absolute z-[150] pointer-events-none transition-all duration-200 ease-out"
        style={{ 
          top: `${top}%`, 
          left: `${left}%`,
      ...
      >
        <div className="bg-zinc-950/95 backdrop-blur-md border border-white/10 p-2.5 rounded-xl shadow-[0_20px_50px_rgba(0,0,0,0.5)] flex flex-col gap-0.5 min-w-[100px] animate-in zoom-in-95 fade-in duration-200">
          <div className="flex items-center gap-1.5 mb-1">
            <div className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: CLUB_COLORS[shot.club] || DEFAULT_COLOR }} />
            <span className="text-[9px] font-black text-white uppercase tracking-tight">{shot.club}</span>
          </div>
          <div className="flex justify-between items-end gap-3">
            <div>
              <p className="text-[6px] font-black text-zinc-500 uppercase tracking-widest">Carry</p>
              <p className="text-xs font-black mono text-emerald-400">{shot.carryDistance.toFixed(1)}<span className="text-[8px] ml-0.5">y</span></p>
            </div>
            <div className="text-right">
              <p className="text-[6px] font-black text-zinc-500 uppercase tracking-widest">Dev</p>
              <p className="text-[10px] font-black mono text-white">
  ...
  return (
    <div className="bg-zinc-900 border border-zinc-800 rounded-3xl p-4 relative flex flex-col group shadow-2xl overflow-hidden">
      ...
      {isExpanded && (
        <div className="fixed inset-0 z-[100] bg-zinc-950 backdrop-blur-3xl flex flex-col animate-in fade-in duration-300">
          {/* Header Bar */}
          <div className="relative shrink-0 flex justify-between items-center p-6 md:p-8 z-[110] bg-gradient-to-b from-zinc-950 via-zinc-950/80 to-transparent pt-[env(safe-area-inset-top,24px)]">
          ...
          {/* Footer / Legend */}
          <div className="shrink-0 p-8 z-[110] bg-gradient-to-t from-zinc-950 via-zinc-950/80 to-transparent pb-[calc(env(safe-area-inset-bottom,24px)+24px)]">
